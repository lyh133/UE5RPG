...
void UAC_RagdollComponent::EndRagdoll()
{
	...
	if (CachedASC && MontageDuration > 0.0f)
	{
		GetWorld()->GetTimerManager().SetTimer(
			RagdollGetUpTimer,
			this,
			&UAC_RagdollComponent::EndRagdollCallBack,
			MontageDuration,
			false
		);
	}
...
void UAC_RagdollComponent::EndRagdollCallBack()
{
	if (CachedASC)
	{
	...
	{
		GEngine->AddOnScreenDebugMessage(
			-1,                 // Key (-1 = new line every time)
			3.0f,               // Display time
			FColor::Green,      // Color
			TEXT("Hello from C++!")
		);
		PCChar->EnablePlayerInput();
	}
